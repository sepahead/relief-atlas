...
**Docset-wide final solution:** `../grandplan.md` §A.8 is the decision record. These prompts are auxiliary asset-generation inputs, not scientific evidence by themselves; any generated output used in experiments must be logged with prompt/config hashes, provider/version metadata, and license/provenance.
